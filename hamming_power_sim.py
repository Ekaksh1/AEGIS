...
class HammingPowerSimulator:
    ...
    def calculate_power(self, current_value: int,
                        previous_value: Optional[int] = None) -> float:
        """
        ...
        # Power model: base + leakage * active_bits +
        # transition_power * transitions
        ...
        return power
    def simulate_sequence(self, data_sequence: np.ndarray,
        ...
        return power_trace
    def simulate_random(self, num_samples: int,
    ...
    def analyze_power_statistics(self,
                                  power_trace: np.ndarray) -> dict:
        """
...
def plot_power_trace(data_sequence: np.ndarray, power_trace: np.ndarray,
    ...
    # Plot data sequence
    ax1.step(range(len(data_sequence)), data_sequence, where='post',
             linewidth=1.5)
...
def demonstrate_simulation():  # pylint: disable=too-many-locals
    ...
    test_sequence = np.array([0x00, 0xFF, 0x00, 0xFF, 0xAA, 0x55, 0xF0,
                              0x0F])
    ...
    data_seq, power_seq = simulator.simulate_random(num_samples=100,
                                                     seed=None)
    ...
    hamming_weights = [simulator.hamming_weight(int(val))
                       for val in data_seq]
    ...
    # Save power trace plot
    plot_filename = traces_dir / f'power_trace_{timestamp}.png'
    plt.savefig(plot_filename, dpi=150, bbox_inches='tight')
    plt.close()  # Close the figure without displaying
    print(f"   Plot saved to '{plot_filename}'")

    # Prepare and save power trace data as JSON
    power_data = {
        "timestamp": timestamp,
        "simulation_config": {
            "bit_width": simulator.bit_width,
            "base_power": simulator.base_power,
            "transition_power": simulator.transition_power,
            "leakage_power": simulator.leakage_power,
        },
        "data_sequence": data_seq.tolist(),
        "power_trace": power_seq.tolist(),
        "statistics": {
    ...
    data_filename = traces_dir / f'power_trace_data_{timestamp}.json'
...
def run_continuous_simulation(stop_event, interval=1.0):
    """Run power simulation continuously and update latest_trace.json."""
    simulator = HammingPowerSimulator()
    traces_dir = Path("traces")
    traces_dir.mkdir(exist_ok=True)

    latest_file = traces_dir / "latest_trace.json"

    while not stop_event.is_set():
        data_seq = np.random.randint(0, 256, size=100)
        power_seq = simulator.simulate_sequence(data_seq)
